...
acqParam.T1_acq_method = 'VFA';  % T1 acquisition method ('VFA' or 'HIFI')
acqParam.B1_correction = 'off'; % B1 correction of DCE sequence
acqParam.isFit = [0 0 1 1 1]; % which acqusitions to fit (fit all for HIFI, last 3 for VFA)
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
 ...
 for i_PS = 1:N_PS 
    ...
    [temp, PS_fit_k1_fast(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
 end
 
PS_means_VFA_fast(:,1) = mean(PS_fit_k1_fast,1)'; % mean for each PS for k = 1
PS_devs_VFA_fast(:,1) = std(PS_fit_k1_fast,0,1)'; % standard deviation at k = 1

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_k1_fast(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_VFA_fast(:,1) = mean(vP_fit_k1_fast,1)'; % mean for each vP for k = 1
vP_devs_VFA_fast(:,1) = std(vP_fit_k1_fast,0,1)'; % standard deviation for k = 1

SeqParam.FA_error = 1.2;
acqParam.FA_true_rads = SeqParam.FA_error * acqParam.FA_nom_rads; % derive actual flip angles for T1 acquisition
% Simulate T1 acquisiton
[PhysParam.T1_blood_meas_s,temp,acqParam.FA_error_meas,temp2] = MeasureT1(PhysParam.S0_blood,PhysParam.T10_blood_s,acqParam,acqParam.T1_acq_method);
[PhysParam.T1_tissue_meas_s,temp,temp2,temp3] = MeasureT1(PhysParam.S0_tissue,PhysParam.T10_tissue_s,acqParam,acqParam.T1_acq_method);
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
 ...
 for i_PS = 1:N_PS 
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_k12_fast(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
 end
 
PS_means_VFA_fast(:,2) = mean(PS_fit_k12_fast,1)'; % add mean for each PS for k = 1.2
PS_devs_VFA_fast(:,2) = std(PS_fit_k12_fast,0,1)'; % add standard deviation for k = 1.2

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_k12_fast(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_VFA_fast(:,2) = mean(vP_fit_k12_fast,1)'; % add mean for each vP for k = 1.2
vP_devs_VFA_fast(:,2) = std(vP_fit_k12_fast,0,1)'; % add standard deviation for k = 1.2

SeqParam.FA_error = 0.8;
acqParam.FA_true_rads = SeqParam.FA_error * acqParam.FA_nom_rads; % derive actual flip angles for T1 acquisition
% Simulate T1 acquisiton
[PhysParam.T1_blood_meas_s,temp,acqParam.FA_error_meas,temp2] = MeasureT1(PhysParam.S0_blood,PhysParam.T10_blood_s,acqParam,acqParam.T1_acq_method);
[PhysParam.T1_tissue_meas_s,temp,temp2,temp3] = MeasureT1(PhysParam.S0_tissue,PhysParam.T10_tissue_s,acqParam,acqParam.T1_acq_method);
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
 ...
 for i_PS = 1:N_PS 
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_k08_fast(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
 end
 
PS_means_VFA_fast(:,3) = mean(PS_fit_k08_fast,1)'; % add mean for each PS for k = 0.8
PS_devs_VFA_fast(:,3) = std(PS_fit_k08_fast,0,1)'; % add standard deviation for k = 0.8

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_k08_fast(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_VFA_fast(:,3) = mean(vP_fit_k08_fast,1)'; % add mean for each vP for k = 0.8
vP_devs_VFA_fast(:,3) = std(vP_fit_k08_fast,0,1)'; % add standard deviation for k = 0.8
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
  ...
  for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_k1_exclude(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_VFA_exclude(:,1) = mean(PS_fit_k1_exclude,1)'; % mean for each PS for k = 1
PS_devs_VFA_exclude(:,1) = std(PS_fit_k1_exclude,0,1)'; % standard deviation for k = 1
...
vP_means_VFA_exclude(:,1) = mean(vP_fit_FXL_exclude,1)'; % mean for each vP for k = 1
vP_devs_VFA_exclude(:,1) = std(vP_fit_FXL_exclude,0,1)'; % standard deviation for k = 1
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
   ...
   for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_k12_exclude(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_VFA_exclude(:,2) = mean(PS_fit_k12_exclude,1)'; % mean for each PS for k = 1.2
PS_devs_VFA_exclude(:,2) = std(PS_fit_k12_exclude,0,1)'; % standard deviation for k = 1.2

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_k12_exclude(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_VFA_exclude(:,2) = mean(vP_fit_k12_exclude,1)'; % add mean for each vP for k = 1.2
vP_devs_VFA_exclude(:,2) = std(vP_fit_k12_exclude,0,1)'; % add standard deviation for k = 1.2
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
 ...
 for i_PS = 1:N_PS 
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_k08_exclude(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
 end
 
PS_means_VFA_exclude(:,3) = mean(PS_fit_k08_exclude,1)'; % add mean for each PS for k = 0.8
PS_devs_VFA_exclude(:,3) = std(PS_fit_k08_exclude,0,1)'; % add standard deviation for k = 0.8

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_k08_exclude(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_VFA_exclude(:,3) = mean(vP_fit_k08_exclude,1)'; % add mean for each vP for k = 0.8
vP_devs_VFA_exclude(:,3) = std(vP_fit_k08_exclude,0,1)'; % add standard deviation for k = 0.8
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
   ...
   for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_k1_slow(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_VFA_slow(:,1) = mean(PS_fit_k1_slow,1)'; % mean for each PS for k = 1
PS_devs_VFA_slow(:,1) = std(PS_fit_k1_slow,0,1)'; % standard deviation for k = 1

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_k1_slow(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_VFA_slow(:,1) = mean(vP_fit_k1_slow,1)'; % mean for each vP for k = 1
vP_devs_VFA_slow(:,1) = std(vP_fit_k1_slow,0,1)'; % standard deviation for k = 1
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
   ...
   for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_k12_slow(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_VFA_slow(:,2) = mean(PS_fit_k12_slow,1)'; % add mean for each PS for k = 1.2
PS_devs_VFA_slow(:,2) = std(PS_fit_k12_slow,0,1)'; % add standard deviation for k = 1.2

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_k12_slow(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_VFA_slow(:,2) = mean(vP_fit_k12_slow,1)'; % add mean for each vP for k = 1.2
vP_devs_VFA_slow(:,2) = std(vP_fit_k12_slow,0,1)'; % add standard deviation for k = 1.2
...
SeqParam.FA_true_deg = SeqParam.FA_error*SeqParam.FA_meas_deg; % true flip angle
   ...
   for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_k08_slow(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_VFA_slow(:,3) = mean(PS_fit_k08_slow,1)'; % add mean for each PS for k = 0.8
PS_devs_VFA_slow(:,3) = std(PS_fit_k08_slow,0,1)'; % add standard deviation for k = 0.8

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_k08_slow(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_VFA_slow(:,3) = mean(vP_fit_k08_slow,1)'; % add mean for each vP for k = 0.8
vP_devs_VFA_slow(:,3) = std(vP_fit_k08_slow,0,1)'; % add standard deviation for k = 0.8
...
errorbar(PS_range, PS_means_VFA_fast(:,2) - PS_range, 1*PS_devs_VFA_fast(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(PS_range, PS_means_VFA_fast(:,3) - PS_range, 1*PS_devs_VFA_fast(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True PS (x10^{-4} min^{-1} )');
ylabel('fitted PS error (x10^{-4} min^{-1} )');
xlim([0 max(PS_range)]);
ylim([-3.2 3.2]);
legend({'k = 1','k = 1.2','k = 0.8'},'Location','southwest')
legend('boxoff')
...
errorbar(PS_range, PS_means_VFA_exclude(:,2) - PS_range, 1*PS_devs_VFA_exclude(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(PS_range, PS_means_VFA_exclude(:,3) - PS_range, 1*PS_devs_VFA_exclude(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True PS (x10^{-4} min^{-1} )');
xlim([0 max(PS_range)]);
ylim([-1.6 1.6]);
...
errorbar(PS_range, PS_means_VFA_slow(:,2) - PS_range, 1*PS_devs_VFA_slow(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(PS_range, PS_means_VFA_slow(:,3) - PS_range, 1*PS_devs_VFA_slow(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True PS (x10^{-4} min^{-1} )');
xlim([0 max(PS_range)]);
ylim([-1.6 1.6]);
...
errorbar(vP_range, vP_means_VFA_fast(:,2) - vP_range, 1*vP_devs_VFA_fast(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(vP_range, vP_means_VFA_fast(:,3) - vP_range, 1*vP_devs_VFA_fast(:,3),'LineWidth',1.3,'Color',Colour3);
...
errorbar(vP_range, vP_means_VFA_exclude(:,2) - vP_range, 1*vP_devs_VFA_exclude(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(vP_range, vP_means_VFA_exclude(:,3) - vP_range, 1*vP_devs_VFA_exclude(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True vP (x10^{-3})');
xlim([0 max(vP_range)]);
ylim([-3.1 3.1]);
...
errorbar(vP_range, vP_means_VFA_slow(:,2) - vP_range, 1*vP_devs_VFA_slow(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(vP_range, vP_means_VFA_slow(:,3) - vP_range, 1*vP_devs_VFA_slow(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True vP (x10^{-3})'); 
xlim([0 max(vP_range)]);
ylim([-3.1 3.1]);
