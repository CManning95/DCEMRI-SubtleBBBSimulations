...
N_vP = size(vP_range,1);
 ...
 PhysParam.FP_mlPer100gPerMin = 18; % high flow
 for i_PS = 1:N_PS 
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_highFp_fast(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
 end
 
PS_means_Fp_fast(:,1) = mean(PS_fit_highFp_fast,1)'; % mean for each PS at high flow
PS_devs_Fp_fast(:,1) = std(PS_fit_highFp_fast,0,1)'; % standard deviation for each PS at high flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_highFp_fast(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_fast(:,1) = mean(vP_fit_highFp_fast,1)'; % mean for each vP at high flow
vP_devs_Fp_fast(:,1) = std(vP_fit_highFp_fast,0,1)'; % standard deviation for each vP at high flow

 PhysParam.FP_mlPer100gPerMin = 9; % med flow
 for i_PS = 1:N_PS 
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_medFp_fast(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
 end
 
PS_means_Fp_fast(:,2) = mean(PS_fit_medFp_fast,1)'; % add mean for each PS at med flow
PS_devs_Fp_fast(:,2) = std(PS_fit_medFp_fast,0,1)'; % add standard deviation for each PS at med flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_medFp_fast(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_fast(:,2) = mean(vP_fit_medFp_fast,1)'; % add mean for each vP at med flow
vP_devs_Fp_fast(:,2) = std(vP_fit_medFp_fast,0,1)'; % add standard deviation for each vP at med flow

PhysParam.FP_mlPer100gPerMin = 4.5; % low flow
 for i_PS = 1:N_PS 
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_lowFp_fast(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
 end
 
PS_means_Fp_fast(:,3) = mean(PS_fit_lowFp_fast,1)'; % add mean for each PS at low flow
PS_devs_Fp_fast(:,3) = std(PS_fit_lowFp_fast,0,1)'; % add standard deviation for each PS at low flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_lowFp_fast(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_fast(:,3) = mean(vP_fit_lowFp_fast,1)'; % add mean for each vP at low flow
vP_devs_Fp_fast(:,3) = std(vP_fit_lowFp_fast,0,1)'; % add standard deviation for each vP at low flow
 ...
 % Generate variable flow PS and vP - fast injection (exclude)
  ...
  for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_highFp_exclude(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_Fp_exclude(:,1) = mean(PS_fit_highFp_exclude,1)'; % mean for each PS at high flow
PS_devs_Fp_exclude(:,1) = std(PS_fit_highFp_exclude,0,1)'; % standard deviation for each PS at high flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_highFp_exclude(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_exclude(:,1) = mean(vP_fit_highFp_exclude,1)'; % mean for each vP at high flow
vP_devs_Fp_exclude(:,1) = std(vP_fit_highFp_exclude,0,1)'; % standard deviation for each PS at high flow

 PhysParam.FP_mlPer100gPerMin = 9; % med flow
   for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_medFp_exclude(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_Fp_exclude(:,2) = mean(PS_fit_medFp_exclude,1)'; % mean for each PS at med flow
PS_devs_Fp_exclude(:,2) = std(PS_fit_medFp_exclude,0,1)'; % standard deviation for each PS at med flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_medFp_exclude(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_exclude(:,2) = mean(vP_fit_medFp_exclude,1)'; % add mean for each vP at med flow
vP_devs_Fp_exclude(:,2) = std(vP_fit_medFp_exclude,0,1)'; % add standard deviation for each vP at med flow

PhysParam.FP_mlPer100gPerMin = 4.5; % low flow
 for i_PS = 1:N_PS 
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_lowFp_exclude(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
 end
 
PS_means_Fp_exclude(:,3) = mean(PS_fit_lowFp_exclude,1)'; % add mean for each PS at low flow
PS_devs_Fp_exclude(:,3) = std(PS_fit_lowFp_exclude,0,1)'; % add standard deviation for each PS at low flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_lowFp_exclude(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_exclude(:,3) = mean(vP_fit_lowFp_exclude,1)'; % add mean for each vP at low flow
vP_devs_Fp_exclude(:,3) = std(vP_fit_lowFp_exclude,0,1)'; % add standard deviation for each vP at low flow
 ...
 SimParam.InputAIFDCENFrames = 69; % number of time points
   ...
   for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_highFp_slow(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_Fp_slow(:,1) = mean(PS_fit_highFp_slow,1)'; % mean for each PS at high flow
PS_devs_Fp_slow(:,1) = std(PS_fit_highFp_slow,0,1)'; % standard deviation for each PS at high flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_highFp_slow(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_slow(:,1) = mean(vP_fit_highFp_slow,1)'; % mean for each vP at high flow
vP_devs_Fp_slow(:,1) = std(vP_fit_highFp_slow,0,1)'; % standard deviation for each vP at high flow

   PhysParam.FP_mlPer100gPerMin = 9; % med flow
   for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_medFp_slow(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_Fp_slow(:,2) = mean(PS_fit_medFp_slow,1)'; % add mean for each PS at med flow
PS_devs_Fp_slow(:,2) = std(PS_fit_medFp_slow,0,1)'; % add standard deviation for each vP at med flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_medFp_slow(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_slow(:,2) = mean(vP_fit_medFp_slow,1)'; % add mean for each vP at med flow
vP_devs_Fp_slow(:,2) = std(vP_fit_medFp_slow,0,1)'; % add standard deviation for each vP at med flow

   PhysParam.FP_mlPer100gPerMin = 4.5; % low flow
   for i_PS = 1:N_PS
    PhysParam.vP = vP_fixed(1);
    PhysParam.PS_perMin = PS_range(i_PS);
    [temp, PS_fit_lowFp_slow(:,i_PS)] = master_single_sim(PhysParam,SeqParam,SimParam);
  end
  
PS_means_Fp_slow(:,3) = mean(PS_fit_lowFp_slow,1)'; % add mean for each PS at low flow
PS_devs_Fp_slow(:,3) = std(PS_fit_lowFp_slow,0,1)'; % add standard deviation for each PS at low flow

for i_vP = 1:N_vP
    PhysParam.PS_perMin = PS_fixed(1);
    PhysParam.vP = vP_range(i_vP);
    [vP_fit_lowFp_slow(:,i_vP),temp] = master_single_sim(PhysParam,SeqParam,SimParam);
end

vP_means_Fp_slow(:,3) = mean(vP_fit_lowFp_slow,1)'; % add mean for each vP for low flow
vP_devs_Fp_slow(:,3) = std(vP_fit_lowFp_slow,0,1)'; % add standard deviation for each vP for low flow
...
errorbar(PS_range, PS_means_Fp_fast(:,2) - PS_range, 1*PS_devs_Fp_fast(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(PS_range, PS_means_Fp_fast(:,3) - PS_range, 1*PS_devs_Fp_fast(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True PS (x10^{-4} min^{-1} )');
ylabel('fitted PS error (x10^{-4} min^{-1} )');
xlim([0 max(PS_range)]);
ylim([-3.2 3.2]);
legend({'F_p = 18','F_p = 9','F_p = 4.5'},'Location','southwest')
legend('boxoff')
...
errorbar(PS_range, PS_means_Fp_exclude(:,2) - PS_range, 1*PS_devs_Fp_exclude(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(PS_range, PS_means_Fp_exclude(:,3) - PS_range, 1*PS_devs_Fp_exclude(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True PS (x10^{-4} min^{-1} )');
xlim([0 max(PS_range)]);
ylim([-1.6 1.6]);
...
errorbar(PS_range, PS_means_Fp_slow(:,2) - PS_range, 1*PS_devs_Fp_slow(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(PS_range, PS_means_Fp_slow(:,3) - PS_range, 1*PS_devs_Fp_slow(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True PS (x10^{-4} min^{-1} )');
xlim([0 max(PS_range)]);
ylim([-1.6 1.6]);
...
errorbar(vP_range, vP_means_Fp_fast(:,2) - vP_range, 1*vP_devs_Fp_fast(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(vP_range, vP_means_Fp_fast(:,3) - vP_range, 1*vP_devs_Fp_fast(:,3),'LineWidth',1.3,'Color',Colour3);
...
errorbar(vP_range, vP_means_Fp_exclude(:,2) - vP_range, 1*vP_devs_Fp_exclude(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(vP_range, vP_means_Fp_exclude(:,3) - vP_range, 1*vP_devs_Fp_exclude(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True vP (x10^{-3})');
xlim([0 max(vP_range)]);
ylim([-3.1 3.1]);
...
errorbar(vP_range, vP_means_Fp_slow(:,2) - vP_range, 1*vP_devs_Fp_slow(:,2),'LineWidth',1.3,'Color',Colour2); hold on;
errorbar(vP_range, vP_means_Fp_slow(:,3) - vP_range, 1*vP_devs_Fp_slow(:,3),'LineWidth',1.3,'Color',Colour3);
xlabel('True vP (x10^{-3})');
xlim([0 max(vP_range)]);
ylim([-3.1 3.1]);
