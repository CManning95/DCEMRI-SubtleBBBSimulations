...
if size(Cp_AIF_mM,2)~=1 && ~strcmp(model,'PatlakFastMultiAIF')
    error('For calls with multiple AIFs use model=PatlakFastMultiAIF');
end
...
switch model %process data using specified model/implementation
    ...
    case 'PatlakFastMultiAIF' %multiple linear regression implementation (recommended)
        ...
        PKP.vP=nan(1,N);
        PKP.PS_perMin=nan(1,N);
        intCp_AIF_mM_min=nan(NTime,size(Cp_AIF_mM,2));
        beta=nan(2,N);
        ...
        for n=1:N
            ...
            CtModelFit_mM(:,n) = [ nan(opts.NIgnore,1) ; reg*beta(:,n) ]; % calculate best fit concentration
        end
